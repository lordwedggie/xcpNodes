...
class xcpDerpFloat  {
    constructor(node) {
        ...
        this.node.prop.min = 0;
        this.node.prop.max = 1;
        this.node.prop.step = 0.01;
        this.node.prop.decimals = 2;
        this.node.prop.defaultValue = 1.0;
        ...
        this.node.prop.colorLabelOutput = "255, 255, 255, 0.90";
        ...
        this.node.prop.colorSliderBG = "005, 005, 010, 0.20";
        ...
        this.node.prop.colorFillbarOn = "128, 255, 128, 0.80";
        this.node.prop.colorFillbarOff = "128, 255, 128, 0.50";
        this.node.prop.colorLRbtn = "128, 128, 128, 0.30";
        this.node.prop.colorLRbtnOn = "128, 128, 128, 0.50";
        this.node.prop.colorLRbtnOff = "128, 128, 128, 0.10";
        this.node.prop.colorBtnStrokeOn = "128, 255, 128, 0.50";
        this.node.prop.colorBtnStrokeOff = "000, 000, 000, 0.20";
        this.node.prop.colorLbtnIndi = "255, 255, 255, 0.50";
        ...
        this.node.prop.labelMarginRight = 4;
        ...
        this.node.prop.derpValue = 0.5;
        this.node.prop.derpTitle = "Derp Float";
        ...
        let LbtnX = 0, RbtnX = 0, LRbtnY = 0, LRbtnH = 0, LRbtnW = 0, fillBarX = 0, fillBarY = 0, fillBarW = 0, fillBarH = 0,
            defaultBtnW = 0, defaultBtnH = 0,defaultBtnX = 0,defaultBtnY = 0;
        let cachedTitle = this.node.title;
        let isConnected = 0;
        let templateSlotColorOn = "255, 255, 255, 0.30";
        let templateSlotColorOff = "000, 000, 000, 0.10";
        ...
        this.node.onAdded = function () {
            ...
            this.outputs[0].color_on = getRGBA(slotUseBtnColor ? colorBtnOn : colorSlotOn);
            this.outputs[0].color_off = getRGBA(slotUseBtnColor ? colorBtnOff : colorSlotOff);
            this.herbina = this.title;
            app.canvas.render_connections_border = false;
            this.onConfigure(); 
        };
        this.node.onPropertyChanged = function (Name) {
            if (!this.configured) return;
            if (Name === "decimals") this.prop.decimals = clamp(this.prop.decimals, 1, 4);
        ...
        this.node.onDrawForeground = function (ctx) {
            ...
            /* -------------------- Text -------------------- */
            let formattedValue = this.prop.derpValue.toFixed(this.prop.decimals);
            ...
            ctx.fillStyle = getRGBA(this.prop.colorLabelOutput);
            ...
            if (this.prop.labelAlign === "right") {
                labelOutputX = canvasWidth - this.prop.marginX - this.prop.canvasWidthOffset - this.prop.padRight;
                ctx.textAlign = "right";
            }
            ctx.fillText(formattedValue, labelOutputX, labelY);
            /* -------------------- SliderBG -------------------- */
            let sliderBGwidth = canvasWidth - this.prop.marginX * 2;
            ...
            if (this.prop.enableLRbtn) {
                ...
                ctx.fill();
                // draw left indicator
                ctx.fillStyle = getRGBA(this.prop.colorLbtnIndi);
            ...
            if (this.prop.enableDefault) {
                ...
                ctx.fillStyle = getRGBA(this.prop.colorLRbtnOff);
                ctx.beginPath();
        ...
        this.node.onMouseMove = function (derp) {
            ...
            let rn = Math.pow(10, this.prop.decimals);
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, _app) {
        if (nodeData.name === "xcpDerpFloat") {
            const onNodeCreated = nodeType.prototype.onNodeCreated;
            nodeType.prototype.onNodeCreated = function () {
                if (onNodeCreated) onNodeCreated.apply(this, []);
                this.xcpDerpFloat = new xcpDerpFloat(this);
            };
        }
    }
});
